...
export function TaskApp() {
  ...
  const handleChangeTask = (task: TaskTypes) =>
    ...
    <>
      <header className="min-h-[300px]">
        <picture className="w-full">
          <source
            ...
            srcSet={theme ? bgDark : bgLight}
          />
          <img
            className="w-full object-cover"
            src={theme ? bgMDark : bgMLight}
            alt="background that contains mountains"
          />
        </picture>
      </header>
      ...
      <main className="lg:-translate-y-60 -translate-y-64 max-w-lg mx-auto flex flex-col z-10 px-5">
        <div className="flex justify-between items-center mb-5">
          <h1 className="lg:text-5xl text-4xl font-semibold tracking-[15px] text-white">
            TODO
          </h1>
          <button onClick={() => toggleTheme(theme)}>
            <img
              className="block"
              src={theme ? iconSun : iconMoon}
              alt="toggle theme"
            />
          </button>
        </div>
        ...
        <TaskList
          ...
          onActiveTasks={handleActiveTasks}
